  ...
  List<double> _calculateColumnWidth(double maxWidth) {
    ...
    for (int i = 0; i < widget.columns!.length; i++) {
      ...
      if (hasRemaining[i]) {
        ...
        double fractionalWidth = 0.0;
        if (fraction > 0.0) {
          fractionalWidth = remainingWidth * fraction;
        ...
        hasRemaining[i] = remainingColumnWidth + epsilon >= math.max(fixedWidth, fractionalWidth);
    ...
    if (delta > epsilon) {
      ...
      if (remainingIndices.isNotEmpty) {
        final perColumn = delta / remainingIndices.length;
